...
export default function InvoicesPage() {
  ...
  const filteredBookings = newInvoice.customer_id
    ? bookings.filter(b => b.customer_id === newInvoice.customer_id && b.status === 'completed')
    : [];
  ...
  function generateInvoicePDF(invoice: Invoice) {
    ...
    doc.text(`Date: ${format(new Date(invoice.invoice_date), 'dd/MM/yyyy')}`, 40, y + 36);
              ...
              {loading ? (
                ...
                <tr key={invoice.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    ...
                    <div className="text-xs text-gray-500 md:hidden">{invoice.booking?.service || 'N/A'}</div>
                  </td>
                  ...
                  <td className="px-4 py-6 whitespace-nowrap text-center">
                    <div className="flex justify-center items-center space-x-1">
                      ...
                      {/* Download Button */}
                      <Tooltip content={downloadingId === invoice.id ? "Generating PDF..." : "Download PDF"} position="top">
                        <button
                      ...
                      {/* Email Button */}
                      <Tooltip content={emailingId === invoice.id ? "Sending Email..." : "Send Invoice by Email"} position="top">
                        <button
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="border-b pb-4">
                ...
                <p className="text-sm text-gray-600">{selectedInvoice.company_name}</p>
                <p className="text-sm text-gray-600">{selectedInvoice.company_address}</p>
                <p className="text-sm text-gray-600">Email: {selectedInvoice.company_email}</p>
                <p className="text-sm text-gray-600">Phone: {selectedInvoice.company_phone}</p>
                <p className="text-sm text-gray-600">VAT: {selectedInvoice.company_vat_number || 'N/A'}</p>
              </div>

              {/* Customer Details */}
              <div className="border-b pb-4">
                <h3 className="font-semibold text-gray-900">Customer Details</h3>
                <p className="text-sm text-gray-600">{selectedInvoice.customer?.name || 'Unknown Customer'}</p>
                <p className="text-sm text-gray-600">{selectedInvoice.customer?.email || 'N/A'}</p>
                <p className="text-sm text-gray-600">{selectedInvoice.customer?.address || 'N/A'}</p>
              </div>
              ...
              <div className="border-b pb-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{selectedInvoice.booking?.service || 'N/A'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{selectedInvoice.due_date ? format(new Date(selectedInvoice.due_date), 'dd/MM/yyyy') : 'N/A'}</p>
                  </div>
                  <div>
              ...
              <div className="flex justify-end space-x-3">
                <Tooltip content={downloadingId === selectedInvoice.id ? 'Generating PDF...' : 'Download PDF'} position="top">
                  <button
                ...
                <Tooltip content={emailingId === selectedInvoice.id ? 'Sending Email...' : 'Send Invoice by Email'} position="top">
                  <button
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] relative">
            ...
            <form className="p-8 space-y-6 overflow-y-auto max-h-[70vh]" onSubmit={handleCreateInvoice}>
              <h2 className="text-2xl font-bold mb-4 text-blue-600">Create New Invoice</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setNewInvoice(prev => ({ ...prev, customer_id: e.target.value, booking_id: '' }))}
                    required
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setNewInvoice(prev => ({ ...prev, booking_id: e.target.value }))}
                    required
                  ...
                  >
                    ...
                    {filteredBookings.map(booking => (
                      <option key={booking.id} value={booking.id}>
                        {booking.service} - {format(new Date(booking.date), 'dd/MM/yyyy')} - £{booking.amount}
                      </option>
                    ))}
                  </select>
                  {newInvoice.customer_id && filteredBookings.length === 0 && (
                    <p className="text-sm text-gray-500 mt-1">No completed bookings found for this customer.</p>
                  )}
                </div>
              ...
              {newInvoice.booking_id && (
                <div className="bg-gray-50 p-4 rounded-lg">
                  <h3 className="font-medium text-gray-900 mb-2">Invoice Preview</h3>
                  {(() => {
                    const selectedBooking = bookings.find(b => b.id === newInvoice.booking_id);
