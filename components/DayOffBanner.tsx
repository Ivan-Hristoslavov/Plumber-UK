...
export function DayOffBanner() {
  ...
  const autoDisableDayOff = async () => {
    try {
      ...
      if (loadError) {
        console.error(
          "Error loading current settings for auto-disable:",
          loadError,
        );
  ...
  if (settings.startDate) {
    ...
    if (today < dayBeforeStart) {
      console.log(
        "Banner not shown: today is more than 1 day before start date",
      );
  ...
  const isAdvanceNotice =
    settings.startDate && today < new Date(settings.startDate);
  const bannerTitle = isAdvanceNotice
    ? "Upcoming Day Off Notice"
    : "Day Off Notice";
                ...
                {(settings.startDate || settings.endDate) && (
                  <div className="hidden md:flex items-center bg-yellow-500 bg-opacity-40 rounded px-2 py-0.5">
                    <span className="text-xs font-medium text-yellow-900">
                      ...
                      {settings.startDate && settings.endDate && " - "}
                      {settings.endDate &&
                        new Date(settings.endDate).toLocaleDateString("en-GB")}
                    </span>
                  </div>
                )}
              </div>
